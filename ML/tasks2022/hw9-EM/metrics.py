...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for ind, elem in enumerate(reference):
        num += len( set(predicted[ind]) & ( set(elem.possible) | set(elem.sure) ) )
        denum += len(predicted[ind])
...
def compute_recall(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for ind, elem in enumerate(reference):
        num += len(set(predicted[ind]) & set(elem.sure))
        denum += len(elem.sure)
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    num_prec, denum_prec = compute_precision(reference, predicted)
    num_rec, denum_rec = compute_recall(reference, predicted)

    return (denum_prec + denum_rec - num_prec - num_rec) / (denum_prec + denum_rec)
